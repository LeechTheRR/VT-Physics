...
namespace VT_Physics::mysph { // cuda kernels
    __global__ void
...
namespace VT_Physics::mysph { // host invoke api
    __host__ void
    ...
    vfsph_div(Data *h_data,
        ...
        if (iter > 100) {
            std::cerr << "MYSPH: div-free iteration exceeds 100 times, crash!\n";
            crash = true;
        }
    }
    ...
    vfsph_incomp(Data *h_data,
        ...
        if (iter > 100) {
            std::cerr << "MYSPH: incomp-iter exceeds 100 times, crash!\n";
            crash = true;
        }
    }
