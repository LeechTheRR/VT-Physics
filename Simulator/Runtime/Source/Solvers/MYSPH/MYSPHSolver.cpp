...
#include "MYSPHCudaApi.cuh"

namespace VT_Physics::mysph {

    MYSPHSolver::MYSPHSolver(uint32_t cudaThreadSize) {
        m_configData = JsonHandler::loadMYSPHConfigTemplateJson();
        m_host_data = new Data();
        m_host_data->thread_num = cudaThreadSize;
        cudaMalloc((void **) &m_device_data, sizeof(Data));
        if (cudaGetLastError() != cudaSuccess) {
            LOG_ERROR("MYSPHSolver fail to allocate memory for device data.")
            delete m_host_data;
            return;
        }

        LOG_INFO("MYSPHSolver Created.");
    }

    json MYSPHSolver::getSolverConfigTemplate() const {
        return JsonHandler::loadMYSPHConfigTemplateJson();
    }

    bool MYSPHSolver::setConfig(json config) {
        m_configData = config;
        if (m_configData.empty()) {
            LOG_ERROR("MYSPH config json is empty.");
            return false;
        }

        if (!checkConfig()) {
            LOG_ERROR("Invalid MYSPH config file.");
            return false;
        }

        auto mysph_config = m_configData["MYSPH"];
        m_host_data->dt = mysph_config["Required"]["timeStep"].get<float>();
        m_host_data->inv_dt = 1 / m_host_data->dt;
        m_host_data->inv_dt2 = m_host_data->inv_dt * m_host_data->inv_dt;
        m_host_data->cur_simTime = 0.f;
        m_host_data->particle_radius = mysph_config["Required"]["particleRadius"].get<float>();
        m_host_data->fPart_rest_density = mysph_config["Required"]["fPartRestDensity"].get<float>();
        m_host_data->bPart_rest_density = mysph_config["Required"]["bPartRestDensity"].get<float>();
        m_host_data->fPart_rest_volume = std::powf(2 * m_host_data->particle_radius, 3);
        m_host_data->h = m_host_data->particle_radius * 4;
        m_host_data->fPart_rest_viscosity = mysph_config["Required"]["fPartRestViscosity"].get<float>();
        m_host_data->div_free_threshold = mysph_config["Required"]["divFreeThreshold"].get<float>();
        m_host_data->incomp_threshold = mysph_config["Required"]["incompThreshold"].get<float>();
        m_host_data->surface_tension_coefficient = mysph_config["Required"]["surfaceTensionCoefficient"].get<float>();
        m_host_data->gravity = make_float3(0.f, -9.8f, 0.f);

        if (mysph_config["Optional"]["enable"]) {
            auto g = mysph_config["Optional"]["gravity"].get<std::vector<float>>();
            m_host_data->gravity = make_float3(g[0], g[1], g[2]);
        }

        LOG_INFO("MYSPHSolver Configured.");
        return true;
    }

    bool MYSPHSolver::setConfigByFile(std::string config_file) {
        setConfig(JsonHandler::loadJson(config_file));
        return true;
    }

    json MYSPHSolver::getSolverObjectComponentConfigTemplate() {
        return JsonHandler::loadMYSPHObjectComponentConfigTemplateJson();
    }

    bool MYSPHSolver::initialize() {
        ...
        if (!ExportUtil::checkConfig(m_configData["EXPORT"])) {
            LOG_ERROR("MYSPHSolver export config not available.");
            return false;
        }

        auto ugns_config = JsonHandler::loadUGNSConfigTemplateJson();
        ugns_config["simSpaceLB"] = m_configData["MYSPH"]["Required"]["simSpaceLB"];
        ugns_config["simSpaceSize"] = m_configData["MYSPH"]["Required"]["simSpaceSize"];
        ugns_config["totalParticleNum"] = m_host_data->particle_num;
        ugns_config["maxNeighborNum"] = m_configData["MYSPH"]["Required"]["maxNeighborNum"];
        ...
        if (!m_host_data->malloc()) {
            LOG_ERROR("MYSPHSolver fail to allocate memory for rt data.");
            return false;
        }
        cudaMalloc((void **) &m_device_data, sizeof(Data));
        ...
        if (cudaGetLastError() != cudaSuccess) {
            LOG_ERROR("MYSPHSolver fail to initialize.")
            return false;
        }
    ...
    bool MYSPHSolver::run() {
        ...
        while (m_host_data->cur_simTime < m_configData["MYSPH"]["Required"]["animationTime"].get<float>()) {
            if (!m_isCrashed) {
                tick();
                exportData();
                LOG_WARNING("MYSPHSolver current frame: " + std::to_string(m_frameCount));
            } else {
                LOG_ERROR("MYSPHSolver is crashed.");
                return false;
            }
        }
    ...
    bool MYSPHSolver::tickNsteps(uint32_t n) {
        if (!m_isInitialized) {
            if (!initialize()) {
                return false;
            }
    ...
    bool MYSPHSolver::attachObject(Object *obj) {
        if (static_cast<uint8_t>(obj->getObjectComponent()->getType()) > 20) {
            LOG_ERROR("MYSPHSolver only support particle geometry yet.");
            return false;
        }

        if (MYSPHSolverSupportedMaterials.count(obj->getObjectComponentConfig()["epmMaterial"]) == 0) {
            LOG_ERROR("MYSPHSolver unsupported material.");
            return false;
        }

        for (const auto &key: MYSPHSolverObjectComponentConfigRequiredKeys) {
            if (!obj->getSolverObjectComponentConfig().contains(key)) {
                LOG_ERROR(
                        "Object: " + std::to_string(obj->getID()) + " has no MYSPH object component config key: " +
                        key);
                return false;
            }
        }

        if (obj->getSolverObjectComponentConfig()["solverType"] != static_cast<uint8_t>(SolverType::MYSPH)) {
            LOG_ERROR("Object: " + std::to_string(obj->getID()) + " has no MYSPH object component config.");
            return false;
        }
    ...
    bool MYSPHSolver::attachObjects(std::vector<Object *> objs) {
        for (auto obj: objs) {
            if (!attachObject(obj)) {
                return false;
            }
    ...
    bool MYSPHSolver::reset() {
        m_configData = JsonHandler::loadMYSPHConfigTemplateJson();
        return true;
    }

    void MYSPHSolver::destroy() {
        m_host_data->free();
        delete m_host_data;
        ...
        LOG_INFO("MYSPHSolver Destroyed.");
    }

    void MYSPHSolver::exportData() {
        ...
        if (exportConfig["SolverRequired"]["enable"] && m_doExportFlag) {
            if (exportConfig["SolverRequired"]["exportGroupPolicy"] == "MERGE") {
                ...
                LOG_ERROR("MYSPHSolver exportGroupPolicy" +
    ...
    bool MYSPHSolver::tick() {
        ...
        if (cudaGetLastError() != cudaSuccess) {
            LOG_ERROR("MYPHSolver tick failed.");
            m_isCrashed = true;
            return false;
        }
        return true;
    ...
    bool MYSPHSolver::checkConfig() const {
        if (!m_configData.contains("MYSPH")) {
            LOG_ERROR("MYSPH config missing main domain: MYSPH")
            return false;
        }

        auto dfsph_config = m_configData["MYSPH"];
        if (!dfsph_config.contains("Required")) {
            LOG_ERROR("MYSPH config missing necessary domain: Required")
            return false;
        }

        for (const auto &key: MYSPHConfigRequiredKeys) {
            if (!dfsph_config["Required"].contains(key)) {
                LOG_ERROR("MYSPH config missing Required key: " + key)
                return false;
            }
        }

        for (const auto &key: MYSPHConfigOptionalKeys) {
            if (!dfsph_config["Optional"].contains(key)) {
                LOG_ERROR("MYSPH config missing Optional key: " + key)
                return false;
            }
        }
